...
assert($('style').text().replace(/\s/g, '').match(/\.item5{.*grid-column:.*}/g));;
return true
}, code)
assert(test)
results.push(true)
...
const test = await page.evaluate((code) => {
window.assert = chai.assert;
 const colStart = getComputedStyle($('.item5')[0]).gridColumnStart; const colEnd = getComputedStyle($('.item5')[0]).gridColumnEnd; const result = colStart.toString() + colEnd.toString(); const correctResults = ['24', '2-1', '2span 2', '2span2', 'span 2-1', '-12', 'span 2span 2', 'span 2auto', 'autospan 2']; assert(correctResults.includes(result)); ;
return true
}, code)
assert(test)
results.push(true)
